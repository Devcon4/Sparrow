          ...
          console.log(event);
            if(event.data.command === 'DumpState'){
                ...
                Object.keys(event.data.payload).forEach(async k => {
                    await idbAccess.set(k, event.data.payload[k]);
                });
            } else if(event.data.command === 'RestoreState') {
                let store = {};

                let dbValues = await idbAccess.all();
                let dbKeys = await idbAccess.keys();

                for(let k in dbKeys) {
                    store[dbKeys[k]] = dbValues[k];
                }

                if(event.ports[0]) {
                    event.ports[0].postMessage({command: 'RestoreState', payload: store});
                }
            } else if (event.data.command === 'ClearDB') {
                await idbAccess.clear();
            }
        });
    }
}

let sw = new lifecycleServiceWorker();
